...
export function MyTipsPage({
    ...
    // Build lookup: match name → breakdown entry (for cell coloring in phase C)
    const breakdownByMatch = new Map(
        (psd?.breakdown ?? []).map((e) => [e.match, e])
    )
        ...
        <div className="page-stack">
            <section className="panel panel-sticky-head page-hero">
                <div>
                    ...
                    <p className="lead-text" style={{ margin: 0 }}>Här ser du exakt vad du har skickat in.</p>
                </div>
                ...
                <section className="panel">
                    <div className="table-wrap">
                        <table className="data-table">
                            ...
                            <tbody>
                                {fixtureTips.map((row) => {
                                    const bk = phase === 'C' ? breakdownByMatch.get(row.match) : undefined
                                    const settled = bk && bk.reason !== 'unsettled'
                                    const scoreHit = settled && bk.reason === 'exact-score'
                                    const signHit = settled && (bk.reason === 'exact-score' || bk.reason === 'correct-sign')
                                    const scoreCls = settled ? (scoreHit ? 'tip-hit' : 'tip-miss') : ''
                                    const signCls = settled ? (signHit ? 'tip-hit' : 'tip-miss') : ''
                                    return (
                                        <tr key={row.match}>
                                            <td data-label="Match">{row.match}</td>
                                            <td data-label="Datum/tid">{row.date}</td>
                                            <td data-label="Resultat" className={scoreCls}>{row.homeScore === '' || row.awayScore === '' ? '—' : `${row.homeScore}-${row.awayScore}`}</td>
                                            <td data-label="1/X/2" className={signCls}>{row.sign || '—'}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
